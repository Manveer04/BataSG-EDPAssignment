...
const UsersPage = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUser(decoded)
        const response = await http.get("/api/staff/customers", { // Use relative URL
          headers: {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await http.delete(`/api/staff/customers/${selectedUserId}`, { // Use relative URL
  ...
  console.log(user)
  if (!user || (user.role !== "standard" && user.role !== "Admin")) {
  ...
  return (
    <div className="users-container" style={{}}>
      <Typography variant="h4" sx={{ fontWeight: "bold", color: "black", textAlign: "center", marginBottom: "20px" }}>Customer List</Typography>
