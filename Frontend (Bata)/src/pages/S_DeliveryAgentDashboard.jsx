...
const SDashboard = () => {
  ...
  const fetchAssignedOrders = (staffId, token) => {
    http
      .get(`/DeliveryAgent/${staffId}/orders`, {
        headers: {
  ...
  const processAssignedOrders = (orders) => {
    ...
    orders.forEach((order) => {
      if (order.orderStatus === 1) { // Shipped
        shipping += 1;
      } else if (order.orderStatus === 2) { // Delivered
        delivered += 1;
      }
    });
    ...
    <Box sx={{ marginTop: "90px", marginLeft: "0%", width: "98.5vw" }}>
      ...
      >
        ...
        >
          ...
          <Typography sx={{ fontWeight: "bold", fontSize: "1.2rem", color: "#1976d2" }}>{shippingCount}</Typography>
        ...
        >
          <Box sx={{ fontSize: "2rem", color: "#1976d2" }}>📬</Box>
          <Typography sx={{ fontWeight: "bold", fontSize: "1.2rem", color: "#1976d2" }}>{deliveredCount}</Typography>
